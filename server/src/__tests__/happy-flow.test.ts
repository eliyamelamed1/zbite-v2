...
describe('Full Happy Flow', () => {
  it('simulates two users interacting end-to-end', async () => {
    ...
    const { recipe: carbonara } = await createTestRecipe(app, userA.token, { title: 'Pasta Carbonara', category: 'Italian' });
    expect(carbonara.title).toBe('Pasta Carbonara');
    expect(carbonara.author.username).toBe('chefanna');

    // ─── 4. USER A creates "Tiramisu" ───
    const { recipe: tiramisu } = await createTestRecipe(app, userA.token, { title: 'Tiramisu', category: 'Desserts' });
    ...
    // ─── 8. Filter by Italian ───
    const exploreItalian = await app.inject({ method: 'GET', url: '/api/recipes/explore?category=Italian' });
    ...
    // ─── 10. USER B likes Carbonara ───
    const like = await app.inject({ method: 'POST', url: `/api/likes/${carbonara._id}`, headers: authHeader(userB.token) });
    expect(like.statusCode).toBe(201);
    const likeStatus = await app.inject({ method: 'GET', url: `/api/likes/${carbonara._id}/status`, headers: authHeader(userB.token) });
    expect(JSON.parse(likeStatus.body).liked).toBe(true);
    ...
    // ─── 12. USER B rates 5 stars ───
    const rate = await app.inject({ method: 'POST', url: `/api/ratings/${carbonara._id}`, headers: authHeader(userB.token), payload: { stars: 5 } });
    expect(JSON.parse(rate.body).averageRating).toBe(5);

    // ─── 13. USER B saves Carbonara ───
    const save = await app.inject({ method: 'POST', url: `/api/saved/${carbonara._id}`, headers: authHeader(userB.token) });
    expect(save.statusCode).toBe(201);

    // ─── 14. USER B follows USER A ───
    ...
    // ─── 17. Filter saved by Italian ───
    const savedItalian = await app.inject({ method: 'GET', url: '/api/saved?category=Italian', headers: authHeader(userB.token) });
    expect(JSON.parse(savedItalian.body).data).toHaveLength(1);

    // ─── 18. USER A checks notifications (5: like, comment, rate, save, follow) ───
    ...
    expect(notifsData).toHaveLength(5);
    const types = notifsData.map((n: any) => n.type).sort();
    expect(types).toEqual(['comment', 'follow', 'like', 'rate', 'save']);

    // ─── 19. Unread count ───
    const unread = await app.inject({ method: 'GET', url: '/api/notifications/unread-count', headers: authHeader(userA.token) });
    expect(JSON.parse(unread.body).count).toBe(5);
    ...
    // ─── 21. Self-rate (no self-notification) ───
    const selfRate = await app.inject({ method: 'POST', url: `/api/ratings/${carbonara._id}`, headers: authHeader(userA.token), payload: { stars: 4 } });
    expect(JSON.parse(selfRate.body).averageRating).toBe(4.5);
    const notifsAfterSelf = await app.inject({ method: 'GET', url: '/api/notifications', headers: authHeader(userA.token) });
    expect(JSON.parse(notifsAfterSelf.body).data).toHaveLength(5); // still 5
    ...
    // ─── 23. USER B views USER A's profile ───
    ...
    // ─── 24-26. Undo: unlike, unsave, unfollow ───
    await app.inject({ method: 'DELETE', url: `/api/likes/${carbonara._id}`, headers: authHeader(userB.token) });
